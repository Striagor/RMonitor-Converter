...
export function getCacheKey(msg: WebJsonMessage): string {
  ...
  switch (id) {
    ...
    case "J":
      return `J:${(msg as WebJsonMessage_J).RegistrationNumber}:${(msg as WebJsonMessage_J).Laptime}`;
    case "F":
      return "F"; // Single instance
    case "DPD":
      return `DPD:${(msg as WebJsonMessage_DPD).TRANSPONDER}:${(msg as WebJsonMessage_DPD).RTC_TIME}`;
    case "DPF":
      return `DPF:${(msg as WebJsonMessage_DPF).Transponder}:${(msg as WebJsonMessage_DPF).RTC_Time}`;
    case "DSI":
